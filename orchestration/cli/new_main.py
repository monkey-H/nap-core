...
from .command import nap_project_from_options
...
log_path = '/home/monkey/Documents/filebrowser'
...
def new_main(path, username, password):
    setup_logging(username)

    command = TopLevelCommand()
    command.nap_sys_dispatch(path, username, password)
...
def setup_logging(username):
    ...
    open('%s/%s/log' % (log_path, username), 'a').close()

    file_handler = logging.FileHandler('%s/%s/log' % (log_path, username))
...
class TopLevelCommand(DocoptCommand):
    ...
    def nap_perform_command(self, options, handler, command_options, path, username, password):
        setup_console_handler(console_handler, options.get('--verbose'))
        ...
        project = nap_project_from_options(path, options, username, password)
        with friendly_error_message():
            handler(project, command_options)
